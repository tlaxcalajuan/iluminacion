...
void CNF_Distance(unsigned int dist_ON){
    if(dist_ON > 0 && dist_ON <= 400){
        distance_to_ON = dist_ON;
    }else{
        distance_to_ON = distance_to_ON;
    }
}


void CNF_Time(unsigned int tiempo_ON){
    if(tiempo_ON > 0 && tiempo_ON <= 15){
        time_ON = tiempo_ON;
    }else{
        time_ON = time_ON;
    }
...
void CNF_Default(){
    ...
    Qg = 0;
    Qb = 255;
    EEPROM_Save(127,Qd,Qt,Qr,Qg,Qb);
    CNF_Color(Qr,Qg,Qb);
}
